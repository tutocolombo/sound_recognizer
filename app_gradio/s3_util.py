...
def to_s3(bucket, audio_bytes, feedback=None, filetype=None):
    key = feedback + _make_key(audio_bytes, filetype=filetype)

    s3_uri = _get_uri_of(bucket, key)

    with open(s3_uri, "wb") as s3_object:
        s3_object.write(audio_bytes)
...
def _get_url_of(bucket, key=None):
...
def _get_uri_of(bucket, key=None):
...
def _make_identifier(byte_data):
