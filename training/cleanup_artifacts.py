...
DEFAULT_PROJECT = "sound-recognizer"
...
def main(args):
    ...
    runs = _get_runs(
        project_path, args.run_ids, args.run_name_res, verbose=args.verbose
    )
...
def clean_run_artifacts(
    run, selector, protect_aliases=True, verbose=False, dryrun=True
):
...
def _get_run_by_id(project_path, run_id, verbose=False):
    ...
    if verbose:
        print(
            f"selecting run {run.entity}/{run.project}/{run.id} with display name {run.name}"
        )
    return run
...
def _get_runs_by_name_re(project_path, run_name_re, verbose=False):
    matching_runs = api.runs(
        path=project_path, filters={"display_name": {"$regex": run_name_re}}
    )
